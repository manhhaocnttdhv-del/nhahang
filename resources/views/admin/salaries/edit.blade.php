...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2><i class="bi bi-cash-coin"></i> Sửa Bảng Lương</h2>
        <a href="{{ route('admin.salaries.show', $salary->id) }}" class="btn btn-secondary">Quay lại</a>
    </div>
                    ...
                    <form action="{{ route('admin.salaries.update', $salary->id) }}" method="POST" id="salaryForm">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="user_id" class="form-select @error('user_id') is-invalid @enderror" required id="user_id">
                                ...
                                @foreach($staff as $s)
                                    <option value="{{ $s->id }}" {{ old('user_id', $salary->user_id) == $s->id ? 'selected' : '' }}
                        ...
                        <div class="mb-3">
                            ...
                            <select name="employment_type" class="form-select @error('employment_type') is-invalid @enderror" required id="employment_type">
                                <option value="full_time" {{ old('employment_type', $salary->employment_type) == 'full_time' ? 'selected' : '' }}>Full-time</option>
                                <option value="part_time" {{ old('employment_type', $salary->employment_type) == 'part_time' ? 'selected' : '' }}>Part-time</option>
                            </select>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="number" name="base_salary" class="form-control @error('base_salary') is-invalid @enderror" 
                                               value="{{ old('base_salary', $salary->base_salary) }}" min="0" step="0.01" id="base_salary">
                                        @error('base_salary')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="number" name="working_days" class="form-control @error('working_days') is-invalid @enderror" 
                                               value="{{ old('working_days', $salary->working_days) }}" min="0" max="31">
                                        @error('working_days')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="number" name="working_hours" class="form-control @error('working_hours') is-invalid @enderror" 
                                               value="{{ old('working_hours', $salary->working_hours) }}" min="0" step="0.5" id="working_hours">
                                        @error('working_hours')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="number" name="hourly_rate" class="form-control @error('hourly_rate') is-invalid @enderror" 
                                               value="{{ old('hourly_rate', $salary->hourly_rate) }}" min="0" step="0.01" id="hourly_rate">
                                        @error('hourly_rate')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                        ...
                        <div class="mb-3">
                            <div class="alert alert-info">
                                <strong>Tổng lương: <span id="total_salary_display">{{ number_format($salary->total_salary, 0, ',', '.') }}</span> đ</strong>
                                <input type="hidden" name="total_salary" id="total_salary" value="{{ $salary->total_salary }}">
                            </div>
                        </div>

                        <div class="mb-3">
                            <label class="form-label">Ghi chú</label>
                            <textarea name="notes" class="form-control @error('notes') is-invalid @enderror" rows="3">{{ old('notes', $salary->notes) }}</textarea>
                            @error('notes')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
